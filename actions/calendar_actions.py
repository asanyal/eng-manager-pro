...
class GetMyDay(ActionInterface):
    ...
    def show_highlights(self, st_first_event, et_last_event, skippable_time):
        st.markdown("<h3>Here's your day</h3>", unsafe_allow_html=True)
        if st_first_event is not None:
            ...
            st.markdown(f"<li><span style='color: #FF6100;'>Wake up at <b>{wake_up_at_time.strftime('%I.%M %p')}</b></span></li>", unsafe_allow_html=True)
            st.markdown(f"<li><span style='color: #FF6100;'>By done with gym by <b>{end_gym_time.strftime('%I.%M %p')}</b></span></li>", unsafe_allow_html=True)
            st.markdown(f"<li><span style='color: #FF6100;'>Head to work by <b>{leave_home_at_time.strftime('%I.%M %p')}</b></span></li>", unsafe_allow_html=True)
